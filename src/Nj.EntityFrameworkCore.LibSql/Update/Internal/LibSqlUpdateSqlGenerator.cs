...
/// </summary>
public class LibSqlUpdateSqlGenerator : UpdateAndSelectSqlGenerator
{
        ...
        string? schema)
    {
        if (columnModification.JsonPath is not (null or "$"))
        {
            ...
            if (columnModification.Property is { IsPrimitiveCollection: false })
            {
                ...
                // LibSqlModificationCommand converted the .NET bool to a "true"/"false" string, here we add the enclosing json().
