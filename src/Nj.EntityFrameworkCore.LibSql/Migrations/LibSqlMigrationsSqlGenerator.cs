...
{
    /// <summary>
    ///     Creates a new <see cref="LibSqlMigrationsSqlGenerator" /> instance.
    /// </summary>
    ...
    private bool IsSpatialiteColumn(AddColumnOperation operation, IModel? model)
        => LibSqlTypeMappingSource.IsSpatialiteType(
        ...
        IModel? model)
    {
        ...
        foreach (var (key, rebuildContext) in rebuilds)
        {
            ...
            foreach (var column in table.Columns)
            {
                ...
                var isAutoincrement = column.FindAnnotation(LibSqlAnnotationNames.Autoincrement)?.Value as bool? == true;
    ...
    {
        if (operation[LibSqlAnnotationNames.InitSpatialMetaData] as bool? != true
            || operation.OldDatabase[LibSqlAnnotationNames.InitSpatialMetaData] as bool? == true)
        {
            return;
        }
    ...
    protected override void Generate(AddColumnOperation operation, IModel? model, MigrationCommandListBuilder builder, bool terminate)
    {
        ...
        var srid = operation[LibSqlAnnotationNames.Srid] as int? ?? 0;
    ...
    protected override void Generate(RenameIndexOperation operation, IModel? model, MigrationCommandListBuilder builder)
        => throw new NotSupportedException(
            LibSqlStrings.InvalidMigrationOperation(operation.GetType().ShortDisplayName()));
        ...
        bool terminate = true)
    {
        ...
        if (operation.PrimaryKey?.Columns.Length == 1)
        {
            ...
            if (columnOp != null)
            {
                columnOp.AddAnnotation(LibSqlAnnotationNames.InlinePrimaryKey, true);
                if (!string.IsNullOrEmpty(operation.PrimaryKey.Name))
                {
                    columnOp.AddAnnotation(LibSqlAnnotationNames.InlinePrimaryKeyName, operation.PrimaryKey.Name);
                }
        ...
        MigrationCommandListBuilder builder)
    {
        ...
        var inlinePk = operation[LibSqlAnnotationNames.InlinePrimaryKey] as bool?;
        if (inlinePk == true)
        {
            var inlinePkName = operation[
                LibSqlAnnotationNames.InlinePrimaryKeyName] as string;
            ...
            var autoincrement = operation[LibSqlAnnotationNames.Autoincrement] as bool?
                // NB: Migrations scaffolded with version 1.0.0 don't have the prefix. See #6461
                ?? operation[LibSqlAnnotationNames.LegacyAutoincrement] as bool?;
    ...
    protected override void Generate(RestartSequenceOperation operation, IModel? model, MigrationCommandListBuilder builder)
        => throw new NotSupportedException(LibSqlStrings.SequencesNotSupported);
